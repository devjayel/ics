    ...
    public function run(): void
    {
        ...
        foreach ($operations as $index => $operation) {
            ...
            $icsRecord = Ics211Record::create([
                ...
                'name' => $operation['name'],
                'type' => rand(1,5),
                'order_request_number' => 'ICS-' . $startDate . '-' . str_pad($index + 1, 3, '0', STR_PAD_LEFT),
                'checkin_location' => $checkinLocations[array_rand($checkinLocations)],
                'start_date' => $startDate,
                'start_time' => sprintf('%02d:%02d:00', rand(6, 14), rand(0, 59)),
            ...
            // Attach 1-3 random operators to the ICS record
            $operatorCount = rand(1, 3);
            $randomOperators = Rul::inRandomOrder()->limit($operatorCount)->pluck('id')->toArray();
            $icsRecord->operators()->attach($randomOperators);
